class Solution:
    ...
    def snakesAndLadders(self, board: List[List[int]]) -> int:
        ...
        # best_cost = defaultdict(lambda: float("inf")) # visited set | keeps track of best cost so far!
        # best_cost[1] = 0
        visited = set()

        for square in range(1, N**2 + 1):
            row_index, col_index = self.getSquareCoordinates(square)
            print(f"{square} -> {board[row_index][col_index]}")
        # return -1
        ...
        while fringe:
            ...
            #     if new_cost >= visited[next_square]:
            #         continue # Not worth visiting again, since smaller cost!
            ...
            for next_square in range(curr_square + 1, min(curr_square + 6, GOAL_STATE) + 1):
                ...
                heapq.heappush(fringe, (cost + 1, final_square))
