...
class Solution:
    ...
    def smallestChair(self, times: List[List[int]], targetFriend: int) -> int:
        ...
        # theoretically allow us to solve the problem in O(nlogn) time w/ AVL tree, but 
        # technically O(n^2) time when unlucky with BST. To minimize this inconvenience, the tree 
        # will be initially balanced, but since I'm not writing a self-balancing tree algorithm,
        # worst case is still TECHNICALLY O(n^2).
